...
function VideoCard({ video, isActive, user, onShare, onOpenComments }) {
  ...
  const isDraggingRef = useRef(false); // REFERENCIA PARA EL ARRASTRE
  ...
  // BUCLE DE ANIMACIÓN DE LA BARRA (60FPS)
  useEffect(() => {
    ...
    const updateLoop = () => {
      // IMPORTANTE: NO ACTUALIZAR SI ESTAMOS ARRASTRANDO (isDraggingRef)
  ...
  // === LÓGICA DE BARRA DE PROGRESO RECUPERADA (SCRUBBING) ===
  ...
  const handleScrubStart = (e) => {
    e.stopPropagation(); // Evita pausar/mutear
    isDraggingRef.current = true;
    
    const clientX = e.touches ? e.touches[0].clientX : e.clientX;
    const ratio = calculateProgress(clientX);
    ...
    // Listeners globales para arrastre suave
    if (e.type === 'mousedown') {
    ...
    >
      ...
      <div className="right-sidebar">
        ...
        <div className="sidebar-item">
          <button className="sidebar-btn" onMouseUp={toggleLikeSidebar} onTouchEnd={toggleLikeSidebar}>
            {isLiked ? <FaHeart style={{color:'#fe2c55'}}/> : <FiHeart/>}
          </button>
          <span className="sidebar-label">{formatK(stats.likes)}</span>
        </div>
        <div className="sidebar-item"><button className="sidebar-btn" onMouseUp={(e)=>{e.stopPropagation(); onOpenComments(video.cloudinaryId)}} onTouchEnd={(e)=>{e.stopPropagation(); onOpenComments(video.cloudinaryId)}}><FiMessageCircle/></button><span className="sidebar-label">{formatK(stats.commentsCount)}</span></div>
        <div className="sidebar-item"><button className="sidebar-btn" onMouseUp={(e)=>{e.stopPropagation(); onShare(video.url)}} onTouchEnd={(e)=>{e.stopPropagation(); onShare(video.url)}}><FiShare2/></button><span className="sidebar-label">Share</span></div>
        <div className="sidebar-item"><button className="sidebar-btn" onMouseUp={(e)=>{e.stopPropagation(); setShowInstaPopup(true)}} onTouchEnd={(e)=>{e.stopPropagation(); setShowInstaPopup(true)}}><FiPlus/></button></div>
      </div>
      ...
      <div className="bottom-info">
        ...
        <div className="description-container">
          ...
          <button className="toggle-more-btn" onMouseUp={(e)=>{e.stopPropagation();setIsDescExpanded(!isDescExpanded)}} onTouchEnd={(e)=>{e.stopPropagation();setIsDescExpanded(!isDescExpanded)}}>{isDescExpanded?"Ocultar":"Ver más"}</button>
        </div>
      </div>
      
      {/* BARRA DE PROGRESO CON EVENTOS RECUPERADOS */}
      <div 
...
export default function App() {
      ...
      try {
        ...
        const formatted = data.resources.map((res, i) => {
          ...
          return {
            ...
            url: `https://res.cloudinary.com/${CLOUD_NAME}/video/upload/${res.public_id}.${res.format}`,
            description: desc
          };
        });
  ...
  const handleScroll = (e) => {
    ...
    const i = Math.round(e.target.scrollTop / h);
    if (i !== currentVideoIndex) setCurrentVideoIndex(i);
  };
